...
using Rose.VExtension.Server.ArgumentsProviders;
...
namespace Rose.VExtension.Server.Models
{
    ...
    public class PluginsExecutor : IPluginsExecutor, IPluginAsyncExecutor
    {
        ...
        private PluginExecutionResult Run(PluginRequestModel request,  PluginSystem.Plugin plugin)
        {
            ...
            firstHandler.Arguments.Source = new PluginArgumentsSource(plugin, request, request.CreatePluginRequest(), firstHandler);

            firstHandler.Arguments.Source.OnAcivityExceutionBegin();
            var response = firstHandler.Activity.Execute(request.CreatePluginRequest());
            firstHandler.Arguments.Source.OnActivityExceutionEnd();
